from utils import read_data, thread_train_setup, train_input_setup, test_input_setup, save_params, merge, array_image_save
...
class FSRCNN(object):
  ...
  def build_model(self):
    ...
    self.weights = {
      'w1': tf.Variable(tf.random_normal([5, 5, self.c_dim, s], stddev=0.0378, dtype=tf.float32), name='w1'),
      ...
      deconv_weight: tf.Variable(tf.random_normal([9, 9, self.c_dim, s], stddev=0.0001, dtype=tf.float32), name=deconv_weight)
    }
  ...
  def run_train(self):
    ...
    for ep in range(self.epoch):
      ...
      for idx in range(0, batch_idxs):
        ...
        # print("self.train_op.shape", self.train_op.shape)



        dictionary = {self.images: batch_images, self.labels: batch_labels, self.batch: self.batch_size}
        tuple = [self.train_op, self.loss]
        print("tuple", tuple)
        _, err = self.sess.run(tuple, feed_dict=dictionary)
        batch_average += err

        if counter % 10 == 0:
          print("Epoch: [" + str(ep+1)  + "], step: [" + str(counter) + "], time: [" + str(time.time() - start_time) + "], loss: [" + str(err) + "]")
